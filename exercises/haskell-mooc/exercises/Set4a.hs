...
longest (x:y:xs)
  ...
  |otherwise              =
    if head x < head y  then longest (x:xs) else longest (y:xs)
...
incrementKey gk = map (\(x, y) -> if gk == x then (x, y+1) else (x, y))
{- incrementKey a [] = []
incrementKey a ((x,y):xs) = if a == x  then (x, y+1) : incrementKey a xs
                            else (x, y) : incrementKey a xs -}
...
winner scores player1 player2 = if f player1 >= f player2 then player1 else player2
   where f p = Map.findWithDefault 0 p scores
...
freqs [] = Map.empty
freqs xs = foldr (Map.alter add1) Map.empty xs

add1 :: Num a => Maybe a -> Maybe a
...
transfer from to amount bank = if Map.notMember from bank ||
...
maxIndex = fst . foldl1 (\(i1, v1) (i2, v2) -> if v1 > v2 then (i1, v1) 
                                              else (i2, v2)) . assocs
