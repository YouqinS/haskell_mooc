...
member x (Set s) = x `elem` s --elem check whether the supplied list contains a specific element or not

-- add a member to a set
add :: Ord a => a -> Set a -> Set a
add x (Set s)
  | x `elem` s  = Set s
  | otherwise = Set (sort (x : s))
...
data State = Start | EggsIn | FlourIn | SugarIn | FlourAndSugarIn | Mixed | Error | Finished
  deriving (Eq,Show)
...
step FlourIn AddSugar   = FlourAndSugarIn
step SugarIn AddFlour = FlourAndSugarIn
step FlourAndSugarIn Mix     = Mixed
...
reverseNonEmpty (x :| []) = x:|[]
reverseNonEmpty (x :| xs) = nh :| nt where
  nh = head rs
  nt = tail rs
  rs = reverse (x : xs)
