...
data UpDown = Increase {value :: Int} | Decrease {value :: Int}
...
rgb Green = [0,1,0]
rgb Blue  = [0,0,1]
rgb (Mix color1 color2) = [(color1 + color2)/2 | (color1, color2) <- zip (rgb color1) (rgb color2)]
rgb (Invert color) = [1 - color | color  <- rgb color]
...
data OneOrTwo x = One x | Two x x deriving Show
