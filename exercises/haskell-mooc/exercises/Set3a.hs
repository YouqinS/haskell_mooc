...
capitalize s = unwords (map capitalizeFirst (words s))
  where capitalizeFirst s = toUpper (head s) : tail s
...
whileRight check x = whileRight' (check x)
  where whileRight' (Left a) = a
        whileRight' (Right b) = whileRight check b
...
joinToLength i s = [ a ++ b | a <- s, b <- s, length (a ++ b) == i ]
...
(+|+) :: [a] -> [a] -> [a]
xs +|+ xy = map head (filter (not . null) [xs, xy])
...
multiApp f gs x = f (map (\a -> a x) gs)
------------------------------------------------------------------------------
