...
isOrdered (Node x l r) = isOrdered l && isOrdered r && x >= getMax x l && x <= getMax x r

getMax :: (Ord p) => p -> Tree p -> p
getMax x Empty = x
getMax _ (Node v l r) = max v $ max (getMax v l) (getMax v r)
...
set _ _ (Node x Empty Empty) = Node x Empty Empty
